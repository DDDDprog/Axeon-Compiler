...
impl Lexer {
    ...
    /// Format: __axeon_pack_set_N, __axeon_pack_push_N, __axeon_pack_push_only,
    ///         __axeon_pack_pop, __axeon_pack_reset
    fn try_pragma_pack_token(text: &str) -> Option<TokenKind> {
        if let Some(rest) = text.strip_prefix("__axeon_pack_") {
            if rest == "pop" {
    ...
    /// Format: __axeon_visibility_push_VISIBILITY, __axeon_visibility_pop
    fn try_pragma_visibility_token(text: &str) -> Option<TokenKind> {
        if let Some(rest) = text.strip_prefix("__axeon_visibility_") {
            if rest == "pop" {
