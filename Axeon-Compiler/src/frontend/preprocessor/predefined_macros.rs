...
impl Preprocessor {
    ...
    /// looking for an `include/` directory that contains our bundled headers.
    /// We check for both `stddef.h` (compiler-intrinsic) and `emmintrin.h`
    /// (SIMD intrinsics) so the directory is found even on a fresh install
    /// where only the intrinsic headers are present.
    ...
    pub fn bundled_include_dir() -> Option<PathBuf> {
        /// Return true when `dir` looks like our bundled include directory.
        fn is_bundled(dir: &PathBuf) -> bool {
            dir.join("stddef.h").is_file() || dir.join("emmintrin.h").is_file()
        }
    ...
    /// Dynamically discover GCC include directories for a given triple under a base path.
    ///
    /// Scans `<base>/<triple>/` for numeric version subdirectories (e.g. 10, 11, 12, 13, 14, 15)
    /// and returns paths of the form `<base>/<triple>/<ver>/include` that actually exist,
    /// sorted newest-first so the most recent GCC headers take priority.
    fn discover_gcc_include_dirs(base: &str, triple: &str) -> Vec<PathBuf> {
        let parent = PathBuf::from(base).join(triple);
        let mut versioned: Vec<(u32, PathBuf)> = Vec::new();
        if let Ok(entries) = std::fs::read_dir(&parent) {
            for entry in entries.flatten() {
                let name = entry.file_name();
                let name_str = name.to_string_lossy();
                // Accept entries whose name is purely numeric (e.g. "15", "14", "13")
                // or starts with a number followed by a dot (e.g. "13.2.0").
                let ver_str = name_str.split('.').next().unwrap_or("");
                if let Ok(ver) = ver_str.parse::<u32>() {
                    let inc = entry.path().join("include");
                    if inc.is_dir() {
                        versioned.push((ver, inc));
                    }
                }
            }
        }
    ...
    /// Collect GCC include dirs for a triple across all known base paths, newest first,
    /// deduplicating entries.
    fn gcc_include_dirs_for_triple(bases: &[&str], triple: &str) -> Vec<PathBuf> {
        let mut seen = std::collections::HashSet::new();
        let mut result = Vec::new();
        for base in bases {
            for p in Self::discover_gcc_include_dirs(base, triple) {
    ...
    ///   1. Bundled `include/` — our own compiler-intrinsic headers
    ///      (stddef.h, stdarg.h, stdbool.h, float.h, limits.h, stdint.h, …)
    ///      plus SIMD intrinsic headers.  These are **always** preferred so
    ///      the compiler never requires GCC to be installed.
    ///   2. `/usr/local/include` — site-local overrides.
    ///   3. `/usr/include/x86_64-linux-gnu` — glibc multiarch headers.
    ///   4. `/usr/include` — main system headers (glibc or musl).
    ///   5. GCC include dirs — added only as a last-resort fallback so that
    ///      any GCC-specific headers not covered by our bundled set are still
    ///      found when GCC is installed.  They are intentionally placed AFTER
    ///      the bundled dir so our stddef.h / stdarg.h / etc. take precedence.
    ///
    /// Arch-specific paths (aarch64, riscv64, i686) are added by `set_target`.
    pub(super) fn default_system_include_paths() -> Vec<PathBuf> {
        let mut paths = Vec::new();

        // 1. Bundled compiler-intrinsic + SIMD headers — highest priority.
        if let Some(bundled) = Self::bundled_include_dir() {
            paths.push(bundled);
        }

        // 2–4. Standard system locations.
        let static_candidates = [
            "/usr/local/include",
            // x86_64 multiarch (removed by set_target for other arches)
        ...
        // 5. GCC include dirs — last-resort fallback only.
        // Skipped entirely when our bundled headers already supply stddef.h,
        // stdarg.h, etc., which is the case for a normal Axeon install.
        // Still added so that obscure GCC-specific headers (e.g. cpuid.h,
        // unwind.h) are reachable on systems where GCC is installed.
        let gcc_bases = ["/usr/lib/gcc", "/usr/lib64/gcc"];
        for p in Self::gcc_include_dirs_for_triple(&gcc_bases, "x86_64-linux-gnu") {
            paths.push(p);
        }
        for triple in &["x86_64-redhat-linux", "x86_64-pc-linux-gnu"] {
            for p in Self::gcc_include_dirs_for_triple(&gcc_bases, triple) {
                paths.push(p);
            }
        }
    ...
    pub fn set_target(&mut self, target: &str) {
        match target {
            "aarch64" => {
                ...
                let mut aarch64_inc: Vec<PathBuf> = Vec::new();
                // Dynamically discover cross-compiler GCC headers (newest first)
                let cross_bases = ["/usr/lib/gcc-cross", "/usr/lib/gcc", "/usr/lib64/gcc"];
                for p in Self::gcc_include_dirs_for_triple(&cross_bases, "aarch64-linux-gnu") {
                    aarch64_inc.push(p);
                }
                for p in Self::gcc_include_dirs_for_triple(&cross_bases, "aarch64-redhat-linux") {
                    aarch64_inc.push(p);
                }
                for p in
                    Self::gcc_include_dirs_for_triple(&cross_bases, "aarch64-unknown-linux-gnu")
                {
                    aarch64_inc.push(p);
                }
                // Static sysroot paths
                for s in &[
                ...
                self.insert_arch_paths_after_bundled_owned(&aarch64_inc);
            ...
            "riscv64" => {
                ...
                let mut riscv_inc: Vec<PathBuf> = Vec::new();
                let cross_bases = ["/usr/lib/gcc-cross", "/usr/lib/gcc", "/usr/lib64/gcc"];
                for p in Self::gcc_include_dirs_for_triple(&cross_bases, "riscv64-linux-gnu") {
                    riscv_inc.push(p);
                }
                for p in Self::gcc_include_dirs_for_triple(&cross_bases, "riscv64-redhat-linux") {
                    riscv_inc.push(p);
                }
                for s in &[
                ...
                self.insert_arch_paths_after_bundled_owned(&riscv_inc);
            ...
            "i686" | "i386" => {
                ...
                let mut i686_inc: Vec<PathBuf> = Vec::new();
                let cross_bases = ["/usr/lib/gcc-cross", "/usr/lib/gcc", "/usr/lib64/gcc"];
                for p in Self::gcc_include_dirs_for_triple(&cross_bases, "i686-linux-gnu") {
                    i686_inc.push(p);
                }
                for p in Self::gcc_include_dirs_for_triple(&cross_bases, "i686-redhat-linux") {
                    i686_inc.push(p);
                }
                for p in Self::gcc_include_dirs_for_triple(&cross_bases, "i686-pc-linux-gnu") {
                    i686_inc.push(p);
                }
                for p in Self::gcc_include_dirs_for_triple(&cross_bases, "i386-linux-gnu") {
                    i686_inc.push(p);
                }
                for p in Self::gcc_include_dirs_for_triple(&cross_bases, "i386-redhat-linux") {
                    i686_inc.push(p);
                }
                for s in &["/usr/i686-linux-gnu/include", "/usr/include/i386-linux-gnu"] {
                    let p = PathBuf::from(s);
                    if p.is_dir() {
                        i686_inc.push(p);
                    }
                }
                self.insert_arch_paths_after_bundled_owned(&i686_inc);
    ...
    /// Insert a list of `PathBuf` include paths right after the bundled include directory,
    /// only adding entries that actually exist on disk.
    fn insert_arch_paths_after_bundled_owned(&mut self, arch_paths: &[PathBuf]) {
        ...
        for path in arch_paths {
            if path.is_dir() {
                self.system_include_paths
                    .insert(insert_pos + offset, path.clone());
                offset += 1;
            }
        }
    }
