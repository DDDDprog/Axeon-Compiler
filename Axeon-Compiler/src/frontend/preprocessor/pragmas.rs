...
impl Preprocessor {
    pub(super) fn handle_pragma(&mut self, rest: &str) -> Option<String> {
        ...
        // emit synthetic __axeon_pack_* tokens that the assembler can't parse)
        ...
        // Suppressed in asm mode: synthetic __axeon_visibility_* tokens are C parser-
    ...
    fn handle_pragma_gcc_visibility(&mut self, content: &str) -> Option<String> {
        ...
        if content == "pop" {
            return Some("__axeon_visibility_pop ;\n".to_string());
        }
            ...
            if rest.starts_with('(') {
                ...
                match inner {
                    "hidden" | "default" | "protected" | "internal" => {
                        return Some(format!("__axeon_visibility_push_{} ;\n", inner));
                    }
                    _ => {}
                }
            }
    ...
    fn handle_pragma_pack(&mut self, content: &str) -> Option<String> {
        ...
        if inner.is_empty() {
            // #pragma pack() - reset
            return Some("__axeon_pack_reset ;\n".to_string());
        }

        // Check for push/pop
        if inner == "pop" {
            return Some("__axeon_pack_pop ;\n".to_string());
        }
        ...
        if let Some(rest) = inner.strip_prefix("push") {
            ...
            if rest.is_empty() {
                ...
                return Some("__axeon_pack_push_only ;\n".to_string());
            }
            // #pragma pack(push, N) - push current and set to N (0 means default)
            if let Ok(n) = rest.parse::<usize>() {
                return Some(format!("__axeon_pack_push_{} ;\n", n));
            }
            return None;
        }
        ...
        if let Ok(n) = inner.parse::<usize>() {
            return Some(format!("__axeon_pack_set_{} ;\n", n));
        }
