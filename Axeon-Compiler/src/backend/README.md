...
compilation with no external toolchain.
...
5. Returns the final assembly string, which is then assembled (via the
   builtin assembler) and linked (via the builtin native linker) into
   an ELF executable.
...
Assembler and linker selection is handled automatically by the native backend.

The native backend provides its own `AssemblerConfig` and `LinkerConfig`
for internal toolchain configuration and input validation.
...
```bash
# Standard build (fully self-contained)
cargo build --release
