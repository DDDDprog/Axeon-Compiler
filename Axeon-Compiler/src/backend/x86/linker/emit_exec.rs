...
pub(super) fn emit_executable(
    ...
    // Build a NUL-terminated interp string from the runtime-provided dynlinker path.
    // This replaces the compile-time INTERP constant so the binary works with
    // whatever dynamic linker is actually installed (glibc or musl).
    let interp_bytes: Vec<u8> = {
        let mut v = dynlinker.as_bytes().to_vec();
        v.push(0);
        v
    };
    let interp: &[u8] = &interp_bytes;
