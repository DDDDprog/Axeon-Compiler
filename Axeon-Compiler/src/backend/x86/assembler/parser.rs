...
fn parse_symbol_with_addend(s: &str) -> Option<(String, i64)> {
    let s = strip_sym_parens(s.trim());
    if s.is_empty() {
        return None;
    }

    // Fast-path: symbol followed by an arithmetic expression (no symbols in expr).
    let mut sym_end = 0usize;
    for (idx, ch) in s.char_indices() {
        if idx == 0 {
            if !(ch.is_ascii_alphabetic() || ch == '_' || ch == '.' || ch.is_ascii_digit()) {
                break;
            }
            sym_end = ch.len_utf8();
            continue;
        ...
        if ch.is_ascii_alphanumeric() || ch == '_' || ch == '.' {
            sym_end = idx + ch.len_utf8();
            continue;
        }
        break;
    }
    if sym_end > 0 && sym_end < s.len() {
        let sym = &s[..sym_end];
        if is_label_like(sym) {
            let rest = s[sym_end..].trim_start();
            if let Some(op) = rest.chars().next() {
                if op == '+' || op == '-' {
                    let expr = rest[1..].trim();
                    if let Ok(val) = parse_integer_expr(expr) {
                        let addend = if op == '-' { -val } else { val };
    ...
    if let Some((i, op)) = find_top_level_add_sub(s) {
        let left = s[..i].trim();
        let right = s[i + 1..].trim();
        if !left.is_empty() {
            if let Ok(val) = parse_integer_expr(right) {
                if is_label_like(left) {
                    let addend = if op == '-' { -val } else { val };
                    return Some((left.to_string(), addend));
                }
            }
        }
    }
...
fn try_parse_symbol_diff_compact(expr: &str) -> Option<DataValue> {
    ...
    for (i, c) in expr.char_indices() {
        match c {
            ...
            '-' if depth == 0 && i > 0 => {
                ...
                let (lhs_sym, lhs_add) = parse_symbol_with_addend(lhs)?;
                let (rhs_sym, rhs_add) = parse_symbol_with_addend(rhs)?;
                if !is_label_like(&lhs_sym) || !is_label_like(&rhs_sym) {
                    return None;
                }
                let addend = lhs_add - rhs_add;
                return if addend != 0 {
                    Some(DataValue::SymbolDiffAddend(lhs_sym, rhs_sym, addend))
                } else {
                    Some(DataValue::SymbolDiff(lhs_sym, rhs_sym))
                };
            }
            _ => {}
        }
...
fn try_parse_symbol_offset_expr(s: &str) -> Option<DataValue> {
    let s = s.trim();
    if let Some((i, op)) = find_top_level_add_sub(s) {
        let left = s[..i].trim();
        let right = s[i + 1..].trim();
        ...
        if let Ok(val) = parse_integer_expr(right) {
            if is_label_like(left) {
                let addend = if op == '-' { -val } else { val };
                return Some(DataValue::SymbolOffset(left.to_string(), addend));
            }
        }
        if op == '+' {
            if let Ok(val) = parse_integer_expr(left) {
                if is_label_like(right) {
                    return Some(DataValue::SymbolOffset(right.to_string(), val));
                }
            }
        }
    }
...
fn parse_data_values(s: &str) -> Result<Vec<DataValue>, String> {
    ...
    for part in s.split(',') {
        ...
        // Check for symbol difference (handles compact and parenthesized forms)
        if let Some(dv) = try_parse_symbol_diff_compact(trimmed) {
            vals.push(dv);
            continue;
        }
        ...
        // Check for symbol+offset or symbol-offset (including expression offsets)
        if let Some(val) = try_parse_symbol_offset_expr(trimmed) {
            vals.push(val);
            continue;
        }
...
fn expand_gas_macros_with_state(
    ...
    while i < lines.len() {
        ...
        // .ifb string / .endif (true if blank)
        if trimmed == ".ifb" || trimmed.starts_with(".ifb ") || trimmed.starts_with(".ifb\t") {
            let rest = if trimmed.len() > ".ifb".len() {
                trimmed[".ifb".len()..].trim()
            } else {
                ""
            };
            let cond = rest.is_empty();
            let mut branches: Vec<(bool, Vec<String>)> = vec![(cond, Vec::new())];
            let mut current_idx = 0;
            let mut depth = 1;
            i += 1;
        ...
        // .ifnb string / .endif (true if not blank)
        if trimmed == ".ifnb" || trimmed.starts_with(".ifnb ") || trimmed.starts_with(".ifnb\t") {
            let rest = if trimmed.len() > ".ifnb".len() {
