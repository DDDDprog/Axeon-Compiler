...
pub(super) fn emit_dynamic_executable(
    ...
    let dynlinker_bytes = dynlinker.as_bytes();
    let has_null = dynlinker_bytes.ends_with(b"\0");
    let interp_len = if has_null {
        dynlinker_bytes.len()
    } else {
        dynlinker_bytes.len() + 1
    };
    ...
    // .interp
    out[interp_offset as usize..interp_offset as usize + dynlinker_bytes.len()]
        .copy_from_slice(dynlinker_bytes);
    if !has_null {
        out[interp_offset as usize + dynlinker_bytes.len()] = 0;
    }
