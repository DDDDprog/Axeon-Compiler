...
impl Driver {
    ...
    pub fn parse_cli_args(&mut self, args: &[String]) -> Result<bool, String> {
        ...
        // Store raw args for GCC -m16 and C++ passthrough. We keep everything except
        ...
        for arg in &args[1..] {
            match arg.as_str() {
                ...
                "--version" => {
                    ...
                    let magenta = if no_color { "" } else { "\x1b[35m" };
                ...
                "--info" => {
                    ...
                    println!("   ▓█   ▓██▒ ▒██▒ ▒██▒░▒████▒░ ████▓▒░▒██░   ▓██░");
                    println!("   ▒▒   ▓▒█░ ▒▒ ░ ░▓ ░░░ ▒░ ░░ ▒░▒░▒░ ░ ▒░   ▒ ▒ ");
                    println!("{reset}              {magenta}AXEON PRO{reset} | Version 2.43");
                    println!("Type :help for commands, :quit to exit");
                    println!();
                    println!("🔵 Phase-1 — Minimal 16-bit Backend (Start Here)");
                    println!("Goal: Generate raw real-mode machine code without GCC.");
                    println!("Add: --target x86_16");
                    println!("Backend emits: 16-bit registers, near calls/jumps, no ELF, no libc, flat memory.");
                    println!();
                    println!("🟢 Phase-2 — Upgrade Your Existing Assembler (Don’t Replace)");
                    println!("Extend: src/assembler/");
                    println!("Add support for: .code16, org 0x7C00, bits 16");
                    println!("Output format: -f bin (flat binary)");
                    println!();
                    println!("🟡 Phase-3 — Minimal Flat Linker");
                    println!("Create: axeon-ld --binary");
                    println!("Resolve symbols, place sections, emit bootable image (no ELF yet).");
                    println!();
                    println!("Target: {}", target.triple());
                    println!("Backend: native");
                    return Ok(true);
    ...
    /// Print compiler limitations for the `--limitations` flag.
    /// This provides users with a quick reference to known limitations.
    fn print_limitations() {
        println!("AXEON Compiler Limitations");
        println!("==========================");
        println!();
        println!("CURRENT SCOPE");
        println!("  This release focuses on fast C compilation and the native x86-64/i686");
        println!("  backends. 16-bit real-mode support and flat-binary linking are planned.");
        println!();
        println!("ROADMAP (PLANNED)");
        println!("  Phase 1: Minimal 16-bit x86 backend for boot/kernel entry");
        println!("  Phase 2: Extend built-in assembler (.code16, segments, -f bin)");
        println!("  Phase 3: Minimal flat-binary linker for boot chains");
        println!();
        println!("STATUS");
        println!("  Optimization tiers and some language features are still evolving.");
        println!("  See README.md and DESIGN_DOC.md for details.");
    }
        ...
        while i < args.len() {
            match args[i].as_str() {
                ...
                "-x" => {
                    ...
                    if i < args.len() {
                        ...
                        if lang == "none" {
                            ...
                            return Err("axeon: error: C++ support has been removed. Please use a C compiler or specify 'c' as the language.".to_string());
                        } else {
                ...
                _ => {
                    ...
                    // Reject C++ files - C++ support has been removed
                    if let Err(e) = Self::check_no_cpp(&args[i]) {
                        return Err(e);
                    }
    ...
    ///
    /// Prints built-in linker version info in a GNU ld-compatible format.
    fn run_linker_version_query(_target: &Target, _linker_items: &[String]) {
        // Print GNU ld-compatible version info for build system detection
        println!("GNU ld (AXEON Linker) 2.40");
    }
