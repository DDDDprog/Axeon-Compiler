...
impl Driver {
    ...
    /// C++ support has been removed - this function now returns false.
    /// Use a C compiler like axeon for C files.
    #[deprecated(since = "0.2.0", note = "C++ support has been removed")]
    pub(super) fn is_cpp_source(path: &str) -> bool {
        // C++ support has been removed - always return false
        // This function is kept for API compatibility but will be removed in future
        false
    }

    /// Returns an error message if a file has C++ extension.
    /// Call this to reject C++ files explicitly.
    pub(super) fn check_no_cpp(path: &str) -> Result<(), String> {
        ...
        {
            Err(format!(
                "axeon: error: C++ support has been removed. '{}' is a C++ file. Please use a C file or a C++ compiler.",
                path
            ))
        } else {
            Ok(())
